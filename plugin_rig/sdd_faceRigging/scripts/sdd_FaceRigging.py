...
def sdd_FaceRigging(rootPath):
    ...
    for i in FR.keys():
        ...
        pos=FRUIPos[i]
        if(pos!=None):
            cmds.iconTextButton(item,c='if(cmds.objExists("%s")):cmds.select("%s")'%(item+_skin,item+_skin),st='textOnly',bgc=[0, 0.8, 0],w=12,h=12,p='frIconBtnFL')
    ...
    cmds.checkBoxGrp('frMirrorOptionToCBG',cw3=[80,70,70],ncb=2, l='Mirror Options:', la2=['Transform', 'Rotate'],va2=[1,1])


    # cmds.formLayout('frIconBtnFL',e=1,ap=[['frIconBgI','left',0,0],['frIconBgI','top',0,0],['frIconBgI','bottom',0,0],['frIconBgI','right',0,0]])
    cmds.formLayout('frIconBtnFL',e=1,ap=[['frMirrorOptionToCBG','left',0,25],['frMirrorOptionToCBG','top',0,95]])
    ...
    cmds.button(l="Export",h=30,w=70,p='mirrorShapeRL02',bgc=(1.000,0.000,0.000),c="zzx_importExportCnt('export')")
    cmds.text(p='mirrorShapeRL02',l='')
    cmds.button(l="Import",h=30,w=70,p='mirrorShapeRL02',en=1,bgc=(0.000,0.000,1.000),c="zzx_importExportCnt('import')")
...
#==================================================================================
    ...
    cmds.iconTextCheckBox('frShowAllMeshListITCB',st='textOnly',l='Mesh',w=100,bgc=[0.35,0.35,0.35],cc='sdd_allMeshListShow()',v=1)
    ...
    cmds.tabLayout('frMainRigTL',e=1,tli=[[1,'Joint Template'], [2, 'Rigging'], [3, 'Sdk Define'],[4, 'Corrective'],[5, 'Advanced']])
...
def zzx_importExportCnt(_type):
    ...
    if _type=="export":
        _ctrlPosDict={}
        _ctrlList=cmds.ls('*FK')+cmds.ls('*Ctrl')+cmds.ls('*IK')+cmds.ls('*Pole')+cmds.ls('*CC')+cmds.ls('*cnt')+cmds.ls('*Switch')
...
def zzx_mirrorCnt(_type):
    _from_AnimList=cmds.ls('Left*FK')+cmds.ls('Left*IK')+cmds.ls('Left*Pole')+cmds.ls('Left*CC')+cmds.ls('Left*Switch')+cmds.ls('L*cnt')
    _to_AnimList=cmds.ls('Right*FK')+cmds.ls('Right*IK')+cmds.ls('Right*Pole')+cmds.ls('Right*CC')+cmds.ls('Right*Switch')+cmds.ls('R*cnt')
    if _type=='LtoR':
...
def zzx_zeroCnt():
    _allAnimGrp=cmds.ls('*FK')+cmds.ls('*Ctrl')+cmds.ls('*IK')+cmds.ls('*Pole')+cmds.ls('*CC')+cmds.ls('*cnt')
    _attrLists=['translateX','translateY','translateZ','rotateX','rotateY','rotateZ','Shut','Thumb','Index','Middle','Ring','Pinky','stertch','Heel','Ball','TipToe','Side']
...
def sdd_deleteHeadScale():
    ret=cmds.confirmDialog(t='HeadScale',m='Delete All HeadScale Object?',b=['Yes','No'],db='No',cb='No',ds='No')
    if(ret=='Yes'):
...
def sdd_newCorrectiveBlendShape():
    ...
    if(sdd_checkCtrMir()):
        result=cmds.promptDialog(t='New Blendshape',m='Enter Blendshape Name:',tx=baseName+_bs+'_'+suf,b=['OK','Cancel'],db='OK',cb='Cancel',ds='Cancel')
    else:
        result=cmds.promptDialog(t='New Blendshape',m='Enter Blendshape Name:',tx=baseName+_bs+'_'+suf,b=['Mirror','Single','Cancel'],db='Mirror',cb='Cancel',ds='Cancel')
    ...
    if(len(selI)==1):
        cmds.setDrivenKeyframe(faceBsNode+'.'+bsMeshName,cd=faceSdkHandle+'.'+selI[0],v=0,dv=0,itt='linear',ott='linear')
        cmds.setDrivenKeyframe(faceBsNode+'.'+bsMeshName,cd=faceSdkHandle+'.'+selI[0],v=1,dv=1,itt='linear',ott='linear')
        ...
        cmds.setDrivenKeyframe(faceBsNode+'.'+bsMeshName,cd=condNode+'.ocr',v=0,dv=0,itt='linear',ott='linear')
        cmds.setDrivenKeyframe(faceBsNode+'.'+bsMeshName,cd=condNode+'.ocr',v=1,dv=1,itt='linear',ott='linear')
    ...
    if(sdd_checkCtrMir(result) and sdd_getMirrorName(bsMeshName)!=bsMeshName):
        ...
        if(len(selI)==1):
            cmds.setDrivenKeyframe(faceBsNode+'.'+mirBsMesh,cd=faceSdkHandle+'.'+sdd_getMirrorName(selI[0]),v=0,dv=0,itt='linear',ott='linear')
            cmds.setDrivenKeyframe(faceBsNode+'.'+mirBsMesh,cd=faceSdkHandle+'.'+sdd_getMirrorName(selI[0]),v=1,dv=1,itt='linear',ott='linear')
            ...
            cmds.setDrivenKeyframe(faceBsNode+'.'+mirBsMesh,cd=condNode+'.ocr',v=0,dv=0,itt='linear',ott='linear')
            cmds.setDrivenKeyframe(faceBsNode+'.'+mirBsMesh,cd=condNode+'.ocr',v=1,dv=1,itt='linear',ott='linear')
...
def sdd_deleteCorrectiveBS():
    ret=cmds.confirmDialog(t='Delete Blendshape',m='Delete This ?',b=['This','This And Mirror','Cancel'],db='Cancel',cb='Cancel',ds='Cancel')
    if(ret=='Cancel'):
        return
    selI=cmds.textScrollList('frCrtBsTSL',q=1,si=1)
    if(selI==None):
...
def sdd_newSdkAttr():
    ...
    if not(cmds.objExists(faceSdkHandle)):
        return
    result=cmds.promptDialog(t='New Attribute',m='Enter Attribute Name:',b=['Ok','Cancel'],db='Ok',cb='Cancel',ds='Cancel')
    if(result=='Cancel'):
        return
...
def sdd_createSdkJntRig():
    ...
    lEyelidList=['L_eyelid_In','L_eyelid_UpIn','L_eyelid_DnIn','L_eyelid_Up','L_eyelid_Dn','L_eyelid_UpOut','L_eyelid_DnOut','L_eyelid_Out']
    ...
    #R Eyelid
    rEyelidList=['R_eyelid_In','R_eyelid_UpIn','R_eyelid_DnIn','R_eyelid_Up','R_eyelid_Dn','R_eyelid_UpOut','R_eyelid_DnOut','R_eyelid_Out']
    ...
    #M_mouth_Up
    mouthList=['M_mouth_Up','L_mouth_Up','L_mouth_Corner','L_mouth_Dn','M_mouth_Dn','R_mouth_Dn','R_mouth_Corner','R_mouth_Up']
    for i in mouthList:
        baseName=FR[i]
        Root=baseName+_root
...
def sdd_initDistanceAttr():
    ...
    objList=[FR['L_eyelid_Up']+_cnt,FR['L_eyelid_UpOut']+_cnt,FR['L_eyelid_Out']+_cnt,FR['L_eyelid_DnOut']+_cnt,FR['L_eyelid_Dn']+_cnt]
    dis=sdd_getAvergeDistance(objList)
    cmds.setAttr(attr,dis)
    ...
    objList=[FR['M_mouth_Up']+_cnt,FR['L_mouth_Up']+_cnt,FR['L_mouth_Corner']+_cnt,FR['L_mouth_Dn']+_cnt,FR['M_mouth_Dn']+_cnt]
    dis=sdd_getAvergeDistance(objList)
    cmds.setAttr(attr,dis)
                ...
                for aI in range(len(attrList)):
                    ...
                    for iv in range(len(idxList)):
                        if(attrList[aI] in fAttrList):
                            cmds.setDrivenKeyframe(attr,cd=cdAttr,v=valList[iv]*fVal,dv=idxList[iv],itt='linear',ott='linear')
                        else:
                            cmds.setDrivenKeyframe(attr,cd=cdAttr,v=valList[iv],dv=idxList[iv],itt='linear',ott='linear')
    finally:
       cmds.progressWindow(ep=1)
...
def sdd_getSdkFactor(cnt):
    ...
    browList=[FR['M_brow'],FR['L_brow_a'],FR['L_brow_b'],FR['L_brow_c'],FR['R_brow_a'],FR['R_brow_b'],FR['R_brow_c'],FR['L_temple'],FR['R_temple'],FR['L_eyeSag_Up'],FR['R_eyeSag_Up']]
    eyeBallList=[FR['L_eye_root'],FR['R_eye_root']]
    eyeLidList=[FR['L_eyelid_Up'],FR['L_eyelid_UpOut'],FR['L_eyelid_Out'],FR['L_eyelid_DnOut'],FR['L_eyelid_Dn'],FR['L_eyelid_DnIn'],FR['L_eyelid_In'],FR['L_eyelid_UpIn']]
    eyeAngleList=[FR['L_eyelid_Up']+_root,FR['L_eyelid_Dn']+_root,FR['R_eyelid_Up']+_root,FR['R_eyelid_Dn']+_root]
    noseList=[FR['L_nose'],FR['R_nose'],FR['M_nose'],FR['L_noseFold'],FR['R_noseFold']]
    cheekList=[FR['L_cheek_In'],FR['L_cheek_Up'],FR['L_cheek_Out'],FR['L_cheek'],FR['R_cheek_In'],FR['R_cheek_Up'],FR['R_cheek_Out'],FR['R_cheek'],FR['L_eyeSag_Dn'],FR['R_eyeSag_Dn']]
    mouthList=[FR['M_mouth_Up'],FR['L_mouth_Up'],FR['L_mouth_Corner'],FR['L_mouth_Dn'],FR['M_mouth_Dn'],FR['R_mouth_Dn'],FR['R_mouth_Corner'],FR['R_mouth_Up']]
...
def sdd_eyelidDefultSdk():
    ...
    sdd_setDrivenKeyframe(FR['L_eyelid_Up']+_root+_sdk,'rx',(arcMid+2)*0.7,faceSdkHandle,cdAttr,1,mircdAttr)
    sdd_setDrivenKeyframe(FR['L_eyelid_Dn']+_root+_sdk,'rx',(-arcMid-2)*0.3,faceSdkHandle,cdAttr,1,mircdAttr)
...
def sdd_returnTempNameDirc():
    TName={'_fol':'_fol','_loc':'_loc','_final':'_final','_target':'_target','face':'face','_base':'_base','_bsNode':'_blendShape','_bs':'_bs','_skin':'_skin','M_':'M_','L_':'L_','R_':'R_','_ctrl':'_ctrl','_rig':'_rig','_jnt':'_jnt','_root':'_root','_rot':'_rot','_cur':'_cur','_cnt':'_cnt','_sdk':'_sdk','_grp':'_grp','_anim':'_anim'}
    return TName
